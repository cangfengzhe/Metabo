...
# mRNA abundance ----
mRNA_abun <- read.csv("/Users/lipidong/work/WX/rna_abundance.csv", 
    stringsAsFactors = F)
...
PARS$name2 <- do.call("rbind", strsplit(PARS[, 1], split = "\\."))[, 
    1]
ensg_enst_entrez <- read.csv("/Users/lipidong/work/protein bundunce/data/ENSG_ENST_Entrez.csv", 
    stringsAsFactors = F)
...
pars01[grep("ENSG", pars01[, 7]), 5] <- pars01[grep("ENSG", pars01[, 
    7]), 7]
