...
willeyFullText <- function(keyWord_1,keyWord_2){
  tryCatch({
    ...
    if(num==''){
      ...
      result<-cbind(keyword,result)
      flag<-dbWriteTable(sqlite,'shenglixue',result,append=T)
      logStr<-paste(c('insert into logshengli values ("',keyWord_1,'","',keyWord_2,'",',0,',',nrow(result),')'),collapse = '')
      ...
      for (ii in  1: pageNum ){
        ...
        #print(date())
        flag<-dbWriteTable(sqlite,'shenglixue',result,append=T)
        logStr<-paste(c('insert into logshengli values ("',keyWord_1,'","',keyWord_2,'",',ii,',',nrow(result),')'),collapse = '')
        flag<-dbGetQuery(sqlite,logStr)
    ...
    print(e);
    str<-paste(c('insert into errshengli values("',keyWord_1,'","',keyWord_2,'",',ii,')'),collapse = '');
...
willeyFullTextwater <- function(keyWord_1,keyWord_2){
  tryCatch({
    ...
    if(num==''){
      ...
      result<-cbind(keyword,result)
      flag<-dbWriteTable(sqlite,'shenglixuewater',result,append=T)
      logStr<-paste(c('insert into logshengliwater values ("',keyWord_1,'","',keyWord_2,'",',0,',',nrow(result),')'),collapse = '')
      flag<-dbGetQuery(sqlite,logStr)
      pageNum<-as.integer(num/20)
      # print(date())
      for (ii in  1: pageNum ){
        ...
        #print(date())
        flag<-dbWriteTable(sqlite,'shenglixuewater',result,append=T)
        logStr<-paste(c('insert into logshengliwater values ("',keyWord_1,'","',keyWord_2,'",',ii,',',nrow(result),')'),collapse = '')
        flag<-dbGetQuery(sqlite,logStr)
        print(paste(ii,'/',pageNum,'/',nrow(result)))
      }
    ...
    print(e);
    str<-paste(c('insert into errshengliwater values("',keyWord_1,'","',keyWord_2,'",',ii,')'),collapse = '');
    tmpstr<-dbGetQuery(sqlite,str)
