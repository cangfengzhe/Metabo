...
conMysql <- dbConnect(dbDriver("MySQL"), dbname = "PostgreSQL", user = "root", password = "sql082508")
allTableMysql = dbListTables(conMysql)  #查询所有表

for (ii in 1:length(allTables)) {
    # readStr=c(allTables[ii],'= dbGetQuery(con,'select * from ',allTables[ii],'')') #读取数据库
    # readStr1=paste(readStr,collapse='') eval(parse(text=readStr1))
