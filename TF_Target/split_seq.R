# split cds utr
library(Biostrings)
pos <- read.table('/Users/lipidong/work/protein bundunce/data/sce_gene_position .txt', stringsAsFactors = F)
...
dna_seq <- rbind(as.data.frame(dna_seq, stringsAsFactors = F), as.data.frame(dna_seq_add, stringsAsFactors = F))
dna_seq[,1] <- as.character(dna_seq[,1])

sapply(1: nrow(cds_pos), function(ii){
  cds_pos[ii,5] <<- substr(dna_seq[cds_pos[ii,1],1], cds_pos[ii, 3], cds_pos[ii,4])
  print(ii)
})
...
sapply(1: nrow(utr3_pos), function(ii){
  utr3_pos[ii,5] <<- substr(dna_seq[utr3_pos[ii,1],1], utr3_pos[ii, 3], utr3_pos[ii,4])
  print(ii)
})
...
sapply(1: nrow(utr5_pos), function(ii){
  utr5_pos[ii,5] <<- substr(dna_seq[utr5_pos[ii,1],1], utr5_pos[ii, 3], utr5_pos[ii,4])
  print(ii)
})
filter(utr5_pos, is.na(V5)==T)


unknow <- unique(c(filter(cds_pos,is.na(V5) == T)[,1], filter(utr3_pos,is.na(V5) == T)[,1], filter(utr5_pos,is.na(V5) == T)[,1]))
...
sapply(1: length(unknow), function(ii){
  ...
  fasta <- readLines(url)
  write.table(fasta, file = './data/unknow.fasta', append = T, quote = F, col.names = F, row.names = F)
  print(ii)
})
