...
createCVset <- function(groupN, kfold)
{
...
GetROCLassoFreq <- function(data, cls){
    ...
    if(kfold.origin > 0) {
        kfold <- kfold.origin;
        CV.inx1 <- createCVset(length(inx1), kfold);
        CV.inx2 <- createCVset(length(inx2), kfold);        
    } else {
        kfold <- 10;
    }
    ...
    for (i in 1:kfold) {
        ...
        # To create a formular for a model with large number of independent vars
        xnam <- names(dw.all)[-1];
...
CVTest.LRmodel <- function(data.in, fmla.in, kfold=10, run.stepwise=FALSE)
{
    ...
    grpIndice.case <- createCVset(nrow(dw.case), kfold)
    grpIndice.ctrl <- createCVset(nrow(dw.ctrl), kfold)
...
PlotProbView <- function(imgName, format="png", dpi=72, mdl.inx, show, showPred) {
     ...
     conf.table <<- xtable(table(pred.out, act.cls), caption="Confusion Matrix (Cross-Validation)");
     ...
     if(anal.type == "test"){
        if(!is.null(dataSet$test.data)){
            ...
            conf.mat.test <<- print(xtable(table(test.pred, test.cls), 
...
PlotROC<-function(imgName, format="png", dpi=72, mdl.inx, avg.method, show.conf, show.holdout, focus="fpr", cutoff = 1.0){
    ...
    if(anal.type=="explore" && mdl.inx == 0){ # compare all models
        ...
        lines(perf.avg@x.values[[1]], perf.avg@y.values[[1]], col=cols[1], add=TRUE);
        for(i in 2:length(preds)){
            perf <- performance(preds[[i]], "tpr", "fpr");
            avg <- ComputeAverageCurve(perf, avg.method);
            lines(avg@x.values[[1]], avg@y.values[[1]], col=cols[i], add=TRUE);
        }
