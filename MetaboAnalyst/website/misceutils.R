...
# parse the graph object from the KGML file
# save the parsed graph objects as .rda data for attach
# note: each species will have a single .rda file created
# the file will contain
# 1) a list with pathway name as name and graph object as value
# 2) a met set for enrichment analysis

SetMetPAObj <- function(dirName){
	require(KEGGgraph);
	require(RBGL);
	if(!exists("cmpd.map")){
		cmpd.map<<-read.csv("cmpd_db.csv", header=T, as.is=T);
	}
	if(!exists("path.map")){
		path.map<<-read.csv("new_pathID.csv", header=F, as.is=T);
	}

	folders <- dir(dirName);
	for(i in 1:length(folders)){
		print(folders[i]);
		setGraphList(folders[i]);
	}
}

setGraphList<-function(dirName){
	nm.cp<- dirName;
	graph.list <- list();

	# set up path.ids
	path.inx <- substr(path.map[,1], 0, 3) == dirName;
	path.ids <- path.map[path.inx,1];
	names(path.ids) <- path.map[path.inx,2];

	ms.list<- list();
	rbc.list <- list(); # relative betweenness centrality measures
	dgr.list <- list(); # out degree list
	dirName <- paste("kegg/", dirName,sep="");
	files <- dir(dirName);
	for(i in 1:length(files)){
		print(paste("processing", paste(dirName, "/", files[i], sep="")));

		g <- KEGGpathway2reactionGraph(parseKGML(paste(dirName, "/", files[i], sep="")));
		nds <- nodes(g);
		start.pos <- unlist(gregexpr(":", nds))+1;
    	        nds <- substr(nds, start.pos, nchar(nds));
    		nms <- KEGGID2Name(nds);
    		names(nds) <- nms;
		nodes(g) <- nds;
		graph.list[[i]] <- g;
		...
		rbc <- bc$relative.betweenness.centrality.vertices[1,]; # one-row matrix
		# check for non
		rbc[is.nan(rbc)]<-0;
		if(sum(rbc) > 0){
