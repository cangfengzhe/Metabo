...
CreateStatFooter<-function(){
    ...
    descr <- c("\\section{Data Annotation}\n",
                "Please be advised that MetaboAnalyst also supports metabolomic data annotation.",
                "For NMR, MS, or GC-MS peak list data, users can perform peak identification by searching",
                "the corresponding libraries. For compound concentration data, users can perform",
    ...
    cat("\n\n", file=rnwFile, append=TRUE);
             ...
             "Thank you for using MetaboAnalyst! For suggestions and feedback please contact Jeff Xia",
             "(\\textit{jeff.xia@mcgill.ca}).\n");
...
CreateMBdoc<-function(){
        ...
        descr <- c("\\subsection{Multivariate Empirical Bayes Approach - MEBA}\n",
                ...
                "\n\n",
                paste("Figure", fig.count<<-fig.count+1,"shows the top four time profiles that are most different across biological conditions.\n"),
                "\n");
        ...
        cmdhist<-c(
                    "\\begin{figure}[htp]",
                    "\\begin{center}",
                    paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$MB4,"}", sep=""),
                    "\\caption{Top Four Different Temporal Profiles}",
                    "\\end{center}",
                    paste("\\label{",imgSet$MB4,"}", sep=""),
                    "\\end{figure}"
                    );
