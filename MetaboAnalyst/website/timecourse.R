...
# plot top four profiles (based on MB) for report
PlotTop4MBProfile <- function(){
    # generate only PDF image and record the name
    pdfName <- "Top4TimeProfiles.pdf"
    imgSet$MB4 <<- pdfName;
    cmpdNms <-rownames(analSet$MB$stats)[1:4];
    pdf(file = pdfName, pointsize=10, width=6,height=6);
    par(mfrow=c(2,2));
    for(nm in cmpdNms){
        plotProfile(nm);
    }
...
plotProfile <-function (varName) {
    ...
    legend("topright", legend=unique(lvlVec), horiz=TRUE, lty=1, col=unique(colVec));
