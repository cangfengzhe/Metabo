...
my_db <- src_mysql(dbname = "dr", host = "localhost", user = "root", 
    password = "")
...
sapply(fileList, function(x) {
    ...
    assign(filename, read.csv(paste(c("./data/database/", x), collapse = ""), 
        stringsAsFactors = F))
    
    
    dbWriteTable(conn, filename, value = get(filename), row.names = F, 
        append = T)
})
...
x <- tableList[1]
sqlQuery <- paste(c("select * from ", x, " PROCEDURE ANALYSE()"), 
    collapse = "")
result <- dbGetQuery(conn, sqlQuery)

aa <- read.csv("./data/database/Info_IC50.csv")

dbWriteTable(conn, name = "info_ic50", value = aa, append = T, 
    row.name = F) 
