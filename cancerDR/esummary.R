# NCBI E-utility Esummary write the data to the database
esummary <- function(url, conn, tableName) {
    
    # url 输入的网址链接 e.g.
    # http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=gds&id=4700,4474,4399&retmode=xml&rettype=abstract
    ...
    df <- data.frame(id = gdsid, acc = acc, title = title, summary = sum, 
        type = type)
