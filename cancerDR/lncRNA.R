# To process the lncRNA data associated to 
# the cancer drug resistance from literature
library(dplyr)
...
# colnames(rawLncRNA)
# process data----
# split the strings from cell table separated by "|"
splitLncRNA <-rawLncRNA %>% 
  cSplit('cell_lines', "|", 'long') %>%
  cSplit('disease', "|", 'long') %>%
  cSplit('drug', "|", 'long') %>%
  cSplit('lncRNA', "|", 'long')
