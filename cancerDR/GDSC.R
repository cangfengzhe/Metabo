...
patternID <- '(?<=CellLine/).*?(?=">)'
cellID <- regmatches(outLinedf, gregexpr(patternID, outLinedf, perl = T))

# download the file about  z-score based the above cellID from database----
...
# write to the sqlite ---- 
# in addition, zscore.db$con is equal to the return from  dbConnect function
...
# import the mutation data from GDSC db
mutRaw <- read.csv('./data/gdscMutation.csv', stringsAsFactors = F)
mut <- mutRaw[,-c(2:5)]
View(mutation)
# extract the mutation position and gene from mutRaw3
# regIndex <- grep (pattern = '.*p\\..*', mutRaw[x,])
# regResult <- mutRaw[x,regIndex]
library(reshape2)
...
mutSplit <- cSplit(mutFilter, splitCols = 3, sep = '::', 
                   direction = 'wide') 
mutSplit <- cSplit(mutSplit, splitCols = 3, sep = '|', 
                   direction = 'long') 
