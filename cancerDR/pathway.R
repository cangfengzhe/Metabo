...
nrow(mirAll)
save(miRDB2microRna, miRDB2targetScan, microRna2targetScan, mirAll, 
    file = "mir2target.rdata")
...
sapply(1:nrow(drug), function(ii) {
    tryCatch({
        geneID <- drug2tar[which(drug2tar[, 2] == drug[ii, 2]), 
            4]
        # geneTable <- as.data.frame(table(geneID), stringsAsFactors =
        # F) geneID <- na.omit(unique(geneTable[which(geneTable[,2]>1
        # ),1]))
...
sapply(1:nrow(drug), function(ii) {
    tryCatch({
        geneID <- drug2meth[which(drug2meth[, 2] == drug[ii, 2]), 
            5]
        # geneTable <- as.data.frame(table(geneID), stringsAsFactors =
        # F) geneID <- na.omit(unique(geneTable[which(geneTable[,2]>1
        # ),1]))
