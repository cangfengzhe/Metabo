#qizhi='“Distending-pain" OR "Wandering pain" OR "string pain" OR "Tightness" OR "oppressive pain" OR "stuffiness unstable pain" OR "depression" OR "dysphoria" OR "sigh" OR "Belching" OR "string pulse"'
#qixu='"Secret anguish" OR "vague pain" OR "Dull pain" OR "Dyspnea" OR "Shortness of breath" OR "Hypodynamia" OR "Spiritlessness" OR "Disinclination to say" OR "Dizziness" OR "Symptoms aggravate after activity" OR "spontaneous perspiration" OR "Light tongue" OR "Light tongue" OR "Ptosis of stomach" OR "Ptosis of kidney" OR "ptosis" OR "Weak pulse"'
...
for(ii in  1: length(qixu)){
  ...
  #flag<-grepl(paste(c('\\b',qixu[ii],'\\b'),collapse = ''),greNEI[,3],perl=T)
...
for (ii in 1: nrow(nei)) {
  for (jj in 1: length(qizhi)){
    ...
    tryCatch({
      ...
      prestr=paste(c('"',molName,'"[title/abstract] AND "',meshName,'"[title/abstract]'),collapse='')
      pmid<-downloadPmid(prestr);
      if(is.matrix(pmid)){
        tmp=pmid;       
        neiRes<-rbind(neiRes,tmp);        
...
for (ii in 1: nrow(gene)){
  tryCatch({
    geneName<-gene[ii,2]
    litBool<-grepl(paste(c('\\b',geneName,'\\b|\\(',geneName,'\\)'),collapse = ''),qixuLit[,3],perl=T)
