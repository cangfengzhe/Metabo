...
### `assign`
为数值分配一个名字,解决为字符串赋值的问题,同时可以解决环境变量问题
...
### `get`
获得变量的值,可以是全局环境变量,也可以是其他环境变量(函数也属于变量)但需要envir参数进行限制.`assign`赋值的变量,可以通过`get`获得.
...
test(2) # 执行函数
ls() # 全局环境中的变量
ls(envir = e1) # e2环境中的变量
get('a', envir = e1) # 必须将变量用引号引起
...
### `all.equal`
比较两个R对象是否相同,并返回报告, 可以通过 `tolerance` 调节差异的精确度
...
all.equal(tan(d45), rep(1, 10), tolerance = 0)  # 将 容差设置为0,则为错

```
----

### all & any
 判断给定的逻辑值,如果都为真,则`all`为真,`any`为真;有一者真则`any`为真,有一者假则`all`为假.
