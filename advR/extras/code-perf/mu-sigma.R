...
GetConditionalMu <- function(omega, Sigma.mu, y, method = "svd") {
    nsteps <- length(omega)
    one <- rep(1, nsteps)
    mean.mu.cond <- c(omega + (1/sum(Sigma.mu)) * (Sigma.mu %*% 
        one) * c(nsteps * y - t(one) %*% omega))
    Sigma.mu.cond <- Sigma.mu - (1/sum(Sigma.mu)) * (Sigma.mu %*% 
        one %*% t(one) %*% Sigma.mu)
...
input_jp(rnorm(300, 0.1, 0.1), rnorm(1, 0.02, 0.05), rnorm(1, 0.8, 
    0.1), rnorm(1, 0.8, 0.1)) 
