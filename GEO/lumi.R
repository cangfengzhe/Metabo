# lumi package is used to process the Illumina Infinium 27k and
# 450k methylation microarray data
...
# 导入lumi 新建LumiBatch 实例 因为没有数据的标准差
# se.expr,所以我们建立se.expr为NA的矩阵
betaValue <- as.matrix(betaValue)
pvalue <- as.matrix(pvalue)
se <- matrix(0, nrow(betaValue), ncol(betaValue))
lumiData <- new("LumiBatch", exprs = betaValue, se.exprs = se, 
    detection <- pvalue)
...
if (require(lumiHumanAll.db) & require(annotate)) {
    selDataMatrix <- selDataMatrix[!is.na(getSYMBOL(rownames(selDataMatrix), 
        "lumiHumanAll.db")), ]
}
...
## get significant gene list with FDR adjusted p.values less
## than 0.01
