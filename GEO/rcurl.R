...
"Cookie"="wordpress_test_cookie=WP+Cookie+check; wfvt_1908536264=5495156d36f3c; _gat=1; _ga=GA1.2.11408004.1419038338",
"Host"="cos.name",
"Origin"="http://cos.name",
"Referer"="http://cos.name/cn/?loggedout=true",
...
"user-submit"="",
"user-cookie"="1",
"redirect_to"="http://cos.name/cn/",
"_wpnonce"="28a75be428",
"_wp_http_referer"="/cn/?loggedout=true")
...
temp<- postForm("http://cos.name/cn/wp-login.php",.params=header,
 .opts=list(cookiefile="",debugfunction=d$update,verbose = TRUE),curl=handle,style="post")

d$value()[2] %>% cat()  #显示 response header


# 可以使用handle，以登录模式打开所有该网站页面
test <-getURL('http://cos.name/cn/',curl=handle,debugfunction=d$update,verbose = TRUE)
d$value()[2] %>% cat()
