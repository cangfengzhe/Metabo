...
row.names(classData)<-classData[,1]
classData<-classData[,2:ncol(classData)]
class(classData[,2])
shi<-c(1:11,36,37,39:49,74,75)
xu<-c(25:35,63:73)
he<-c(12:24,50:62)
shiData<-classData[,shi]
shiData2=rbind(shiData[])
xuData<-classData[,xu]
heData<-classData[,he]
View(heData)
...
Nvalue<-matrix(NA,16,4)
for(ii in 1:nrow(classData)){
  Nvalue[ii,1]<-row.names(classData)[ii]
...
#t检验
t.test
tRes<-matrix(NA,nrow(classData),2)
for(ii in 1:nrow(classData)){
  tmp<-data.frame(data=c(as.numeric(shiData[ii,]),as.numeric(xuData[ii,])),
                  type=c(rep('shi',length(shiData[1,])),rep('xu',length(xuData[1,]))))
   lm<-t.test(tmp[,1]~tmp[,2])
  tRes[ii,2]<-lm$p.value
  tRes[ii,1]<-row.names(classData)[ii]
}
View(tRes)
meanRes<-NA
for(ii in 1:nrow(classData)){
...
names(meanRes)<-row.names(classData)
